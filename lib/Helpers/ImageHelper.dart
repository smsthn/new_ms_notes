...
class SaveImageHelper{
  ...
  Future _saveImage({bool takenPhoto = false})async{
    ...
    Image img = i1.width>this.width - 10? copyResize(i1, this.width - 10):i1;
...
class AddImageHandler{
  ...
  Future<List<File>> getResizedImages(int width)async{
    ...
    return images.map((i)=>File(join(temp.path,basename(i.path)))..writeAsBytesSync(encodeJpg(copyResize(decodeImage(i.readAsBytesSync()), ((width/10)-1).toInt())))).toList();
  }
